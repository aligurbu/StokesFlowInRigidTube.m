...
! call: GxpN = NearlySingularIntegrals_GxpN(xi_e, x, mu, s01, s02, ...
...
mwPointer xi_e, x, grx, grw, gtx, gtw, Gx
mwPointer ngr, ngt
...
call getNearSingular_Gx(%val(xi_e), %val(x), mu, &
...
subroutine getNearSingular_Gx(xi_e, x, mu, &
                              s01, s02, thet_min, thet_max, rmax_fn, &
...
double precision :: G(9) ! vector representations of 3x3 matrices
...
do mm = 1,ngt
    ...
    do nn = 1,ngr
        ...
        do kk = 1,9
            Gx(9*kk-8:9*kk) = Gx(9*kk-8:9*kk) + G*(N(kk)*wt)
        enddo
    enddo
enddo
    
end subroutine getNearSingular_Gx
