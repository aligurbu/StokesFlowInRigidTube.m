function [GxN, KxN] = WeaklySingular_ElementIntegrals_GxN_KxN ...
                      (chi, xi_e, mu, xnodenum, grx, grw, gtx, gtw)
%%
% chi           coordinates of target point
% xi_e          matrix of element node coord (3x9)
% mu            viscosity
% xnodenum      node number of target point in element
% grx, grw      gauss node coordinates and weights for radial direction r
% gtx, gtw      gauss node coordinates and weights for polar angle theta 
% GxN, KxN      weakly-singular element integrals
%%
found_mex_WeaklySingularIntegrals_GxN_KxN = ...
            ~isempty(which('Integration/WeaklySingularIntegrals_GxN_KxN'));
if found_mex_WeaklySingularIntegrals_GxN_KxN
    switch xnodenum
        case 1
            zetachi1 = -1; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = 0; 
            thet_max = pi/4; 
            rmax_fn = 1;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = pi/4; 
            thet_max = pi/2; 
            rmax_fn = 2;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 2
            zetachi1 = 1; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = pi/2; 
            thet_max = 3*pi/4; 
            rmax_fn = 3;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 3*pi/4; 
            thet_max = pi; 
            rmax_fn = 4;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 3
            zetachi1 = 1; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = pi; 
            thet_max = 5*pi/4; 
            rmax_fn = 5;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 5*pi/4; 
            thet_max = 3*pi/2; 
            rmax_fn = 6;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 4
            zetachi1 = -1; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = 3*pi/2; 
            thet_max = 7*pi/4; 
            rmax_fn = 7;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 7*pi/4; 
            thet_max = 2*pi; 
            rmax_fn = 8;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 5
            zetachi1 = 0; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = 0; 
            thet_max = atan2(2,1); 
            rmax_fn = 9;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(2,1); 
            thet_max = atan2(2,-1); 
            rmax_fn = 10;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(2,-1); 
            thet_max = pi; 
            rmax_fn = 11;
            GxN_KxN_tr3 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 6
            zetachi1 = 1; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = pi/2; 
            thet_max = atan2(1,-2); 
            rmax_fn = 12;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(1,-2); 
            thet_max = atan2(-1,-2) + 2*pi;
            rmax_fn = 13;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(-1,-2) + 2*pi; 
            thet_max = 3*pi/2;
            rmax_fn = 14;
            GxN_KxN_tr3 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 7
            zetachi1 = 0; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = pi; 
            thet_max = atan2(-2,-1) + 2*pi; 
            rmax_fn = 15;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(-2,-1) + 2*pi; 
            thet_max = atan2(-2,1) + 2*pi;
            rmax_fn = 16;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(-2,1) + 2*pi; 
            thet_max = 2*pi;
            rmax_fn = 17;
            GxN_KxN_tr3 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 8
            zetachi1 = -1; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = -pi/2; 
            thet_max = atan2(-1,2); 
            rmax_fn = 18;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(-1,2); 
            thet_max = atan2(1,2);
            rmax_fn = 19;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(1,2); 
            thet_max = pi/2;
            rmax_fn = 20;
            GxN_KxN_tr3 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 9
            zetachi1 = 0; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = -pi/4; 
            thet_max = pi/4; 
            rmax_fn = 21;
            GxN_KxN_tr1 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = pi/4; 
            thet_max = 3*pi/4;
            rmax_fn = 22;
            GxN_KxN_tr2 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = 3*pi/4; 
            thet_max = 5*pi/4;
            rmax_fn = 23;
            GxN_KxN_tr3 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            % Triangle 4
            thet_min = 5*pi/4; 
            thet_max = 7*pi/4;
            rmax_fn = 24;
            GxN_KxN_tr4 = WeaklySingularIntegrals_GxN_KxN(xi_e, chi, mu,...
                                                          zetachi1, zetachi2, ...
                                                          thet_min, thet_max, ...
                                                          rmax_fn,...
                                                          grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27) + GxN_KxN_tr4(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54) + GxN_KxN_tr4(:,28:54);
    end
else
    switch xnodenum
        case 1
            zetachi1 = -1; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = 0; 
            thet_max = pi/4; 
            rmax_fn = 1;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = pi/4; 
            thet_max = pi/2; 
            rmax_fn = 2;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 2
            zetachi1 = 1; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = pi/2; 
            thet_max = 3*pi/4; 
            rmax_fn = 3;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 3*pi/4; 
            thet_max = pi; 
            rmax_fn = 4;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 3
            zetachi1 = 1; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = pi; 
            thet_max = 5*pi/4; 
            rmax_fn = 5;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 5*pi/4; 
            thet_max = 3*pi/2; 
            rmax_fn = 6;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 4
            zetachi1 = -1; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = 3*pi/2; 
            thet_max = 7*pi/4; 
            rmax_fn = 7;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = 7*pi/4; 
            thet_max = 2*pi; 
            rmax_fn = 8;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54);
        case 5
            zetachi1 = 0; 
            zetachi2 = -1;
            % Triangle 1
            thet_min = 0; 
            thet_max = atan2(2,1); 
            rmax_fn = 9;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(2,1); 
            thet_max = atan2(2,-1); 
            rmax_fn = 10;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(2,-1); 
            thet_max = pi; 
            rmax_fn = 11;
            GxN_KxN_tr3 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 6
            zetachi1 = 1; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = pi/2; 
            thet_max = atan2(1,-2); 
            rmax_fn = 12;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(1,-2); 
            thet_max = atan2(-1,-2) + 2*pi;
            rmax_fn = 13;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(-1,-2) + 2*pi; 
            thet_max = 3*pi/2;
            rmax_fn = 14;
            GxN_KxN_tr3 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 7
            zetachi1 = 0; 
            zetachi2 = 1;
            % Triangle 1
            thet_min = pi; 
            thet_max = atan2(-2,-1) + 2*pi; 
            rmax_fn = 15;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(-2,-1) + 2*pi; 
            thet_max = atan2(-2,1) + 2*pi;
            rmax_fn = 16;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(-2,1) + 2*pi; 
            thet_max = 2*pi;
            rmax_fn = 17;
            GxN_KxN_tr3 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 8
            zetachi1 = -1; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = -pi/2; 
            thet_max = atan2(-1,2); 
            rmax_fn = 18;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = atan2(-1,2); 
            thet_max = atan2(1,2);
            rmax_fn = 19;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = atan2(1,2); 
            thet_max = pi/2;
            rmax_fn = 20;
            GxN_KxN_tr3 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54);
        case 9
            zetachi1 = 0; 
            zetachi2 = 0;
            % Triangle 1
            thet_min = -pi/4; 
            thet_max = pi/4; 
            rmax_fn = 21;
            GxN_KxN_tr1 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 2
            thet_min = pi/4; 
            thet_max = 3*pi/4;
            rmax_fn = 22;
            GxN_KxN_tr2 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 3
            thet_min = 3*pi/4; 
            thet_max = 5*pi/4;
            rmax_fn = 23;
            GxN_KxN_tr3 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            % Triangle 4
            thet_min = 5*pi/4; 
            thet_max = 7*pi/4;
            rmax_fn = 24;
            GxN_KxN_tr4 = ...
                  WeaklySingularIntegrals_GxN_KxN_M(xi_e, chi, mu,...
                                                    zetachi1, zetachi2, ...
                                                    thet_min, thet_max, ...
                                                    rmax_fn,...
                                                    grx, grw, gtx, gtw);
            GxN = GxN_KxN_tr1(:,1:27) + GxN_KxN_tr2(:,1:27) + ...
                  GxN_KxN_tr3(:,1:27) + GxN_KxN_tr4(:,1:27);
            KxN = GxN_KxN_tr1(:,28:54) + GxN_KxN_tr2(:,28:54) + ...
                  GxN_KxN_tr3(:,28:54) + GxN_KxN_tr4(:,28:54);
    end
end
