...
! call: K = WeaklySingularIntegrals_KxN_M(xi_e, x, s01, s02, ...
...
mwPointer xi_e, x, grx, grw, gtx, gtw, K
mwPointer ngr, ngt
...
call getSingular_Kx(%val(xi_e), %val(x), &
...
subroutine getSingular_Kx(xi_e, x, &
                          s01, s02, thet_min, thet_max, rmax_fn, &
...
double precision :: K_(9) ! vector representations of 3x3 matrices
...
do mm = 1,ngt
    ...
    do nn = 1,ngr
        ...
        K_ = Prhat*(3.d0*dot_product(rhat,nhat)/(4.d0*pi))
        ...
        do kk = 1,9
            K(9*kk-8:9*kk) = K(9*kk-8:9*kk) + K_*(Nbar(kk)*wt/denom)
        enddo
    enddo
enddo
    
end subroutine getSingular_Kx
