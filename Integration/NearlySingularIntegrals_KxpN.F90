...
! call: KxpN = NearlySingularIntegrals_KxpN_M(xi_e, x, s01, s02, ...
...
mwPointer xi_e, x, grx, grw, gtx, gtw, Kx
mwPointer ngr, ngt
...
xi_e = mxGetPr(prhs(1))
x = mxGetPr(prhs(2))
s01 = mxGetScalar(prhs(3))
s02 = mxGetScalar(prhs(4))
thet_min = mxGetScalar(prhs(5))
thet_max = mxGetScalar(prhs(6))
rmax_fn = int(mxGetScalar(prhs(7)))
grx = mxGetPr(prhs(8))
grw = mxGetPr(prhs(9))
ngr = max0(mxGetM(prhs(8)),mxGetN(prhs(8)))
gtx = mxGetPr(prhs(10))
gtw = mxGetPr(prhs(11))
ngt = max0(mxGetM(prhs(10)),mxGetN(prhs(10)))

plhs(1) = mxCreateDoubleMatrix(3_8, 27_8, complexFlag)
Kx = mxGetPr(plhs(1))

call getNearSingular_Kx(%val(xi_e), %val(x), mu, &
...
subroutine getNearSingular_Kx(xi_e, x, mu, &
                              s01, s02, thet_min, thet_max, rmax_fn, &
...
double precision :: K(9) ! vector representations of 3x3 matrices

double precision, parameter :: pi = 3.141592653589793d0

double precision :: thet, Jthet, rm, Jr, rr
...
do mm = 1,ngt
    ...
    do nn = 1,ngr
        ...
        wt = ((rr^2*JJ)/normr^2)*Jr*Jthet*grw(nn)*gtw(mm)
        do kk = 1,9
            Kx(9*kk-8:9*kk) = Kx(9*kk-8:9*kk) + K*(Nbar(kk)*wt)
        enddo
    enddo
enddo
    
end subroutine getNearSingular_Kx

!------------------------------------------------------------------------------
    
function rmax(thet, s01, s02, fn_ind)

implicit none

double precision :: thet, s01, s02, rmax
integer :: fn_ind
...
case(8)
    rmax = 2.d0/dcos(thet)
case(9)
    rmax = (1.d0-s01)/dcos(thet)
case(10)
    rmax = 2.d0/dcos(pi/2.d0-thet)
case(11)
    rmax = (s01+1.d0)/dcos(pi-thet)
case(12)
    rmax = (1.d0-s02)/dcos(thet-pi/2.d0)
case(13)
    rmax = 2.d0/dcos(pi-thet)
case(14)
    rmax = (s02+1.d0)/dcos(3.d0*pi/2.d0-thet)
case(15)
    rmax = (s01+1.d0)/dcos(thet-pi)
case(16)
    rmax = 2.d0/dcos(3.d0*pi/2.d0-thet)
case(17)
    rmax = (1.d0-s01)/dcos(thet)
case(18)
    rmax = (s02+1.d0)/dcos(pi/2.d0+thet)
case(19)
    rmax = 2.d0/dcos(thet)
case(20)
    rmax = (1.d0-s02)/dcos(pi/2.d0-thet)
case(21)
    rmax = (1.d0-s01)/dcos(thet)
case(22)
    rmax = (1.d0-s02)/dcos(pi/2.d0-thet)
