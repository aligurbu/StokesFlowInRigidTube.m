...
! call: G = WeaklySingularIntegrals_GxN_M(xi_e, x, mu, s01, s02, ...
...
mwPointer xi_e, x, grx, grw, gtx, gtw, G
mwPointer ngr, ngt
...
call getSingular_Gx_M(%val(xi_e), %val(x), mu, &
...
subroutine getSingular_Gx_M(xi_e, x, mu, &
                            s01, s02, thet_min, thet_max, rmax_fn, &
...
double precision :: G_(9) ! vector representations of 3x3 matrices
...
do mm = 1,ngt
    ...
    do nn = 1,ngr
        ...
        do kk = 1,9
            G(9*kk-8:9*kk) = G(9*kk-8:9*kk) + G_*(N(kk)*wt)
        enddo
    enddo
enddo
    
end subroutine getSingular_Gx_M
